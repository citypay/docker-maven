#!/bin/sh
# ------------------------------------------------------------------------------
# Adapted from Docker Bench for Security 1.3.5 to run inside a container for
# self assessment. Mimics running as "docker/docker-bench-security -t $image -c container_images"
# https://raw.githubusercontent.com/docker/docker-bench-security/master/docker-bench-security.sh
# ------------------------------------------------------------------------------
...
#  if [ -n "$include" ]; then
    ...
    # GF moved to awk $2 to return container name rather than runtime name
    containers=$(docker ps | sed '1d' | awk '{print $2}' | grep -v "$benchcont" | grep -E "$pattern")
